...
- **GORM 지원**: 내장 GORM 커넥터 제공 (example/gorm 참조)
- **database/sql 지원**: 표준 라이브러리와 완벽 호환
...
GORM을 사용하려면 `example/gorm` 패키지를 참조하세요:

```bash
go get github.com/tidylogic/pgtestkit/example/gorm@latest
```

자세한 예제는 [example/gorm](example/gorm) 디렉토리를 참조하세요.
